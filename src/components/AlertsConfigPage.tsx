...
export const AlertsConfigPage: React.FC = () => {
  ...
  return (
    <ConfigPageShell title="Alerts & Monitoring" description="Service health, VMQ alert pipeline status, and iMessage notification monitor" accentColor="yellow" loading={loading} error={error} onRetry={refetch}>
        ...
        <div className="space-y-2">
          {[
            ...
            <motion.div key={cfg.key} variants={rowItem}
              ...
              <div className="w-12 h-6 rounded-full bg-yellow-500/80 shadow-[0_0_12px_rgba(255,255,0,0.3)] relative">
                <span className="absolute top-0.5 w-5 h-5 rounded-full bg-white translate-x-6" />
              </div>
            </motion.div>
          ))}
        </div>
